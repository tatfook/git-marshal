...
describe('test/app/service/file.test.ts', () => {
    ...
    describe('#upsertFile', () => {
        ...
        it('should not upsert file path without filename', async () => {
            try {
                await ctx.service.file.upsertFile(repo.path, 'hello');
    ...
    describe('#deleteFile', () => {
        ...
        it('should not delete file path without filename', async () => {
            try {
                await ctx.service.file.deleteFile(repo.path);
    ...
    describe('#moveFile', () => {
        it('should move file to the new path', async () => {
            const data = await ctx.service.file.moveFile(repo.path + '/hey.txt', repo.path + '/ho.txt');
            assert(data);
        });
        it('should not move file to a different repo', async () => {
            const repo2 = await app.factory.create('repo', {}, { guard });
            try {
                await ctx.service.file.moveFile(repo.path + '/hey.txt', repo2.path + '/ho.txt');
                assert.fail('should fail to move file');
            } catch (e) {
                assert(e.message === 'only support moving files in the same repo');
            }
        });

        it('should not move file to a invalid file path', async () => {
            try {
                await ctx.service.file.moveFile(repo.path + '/hey.txt', repo.path);
                assert.fail('should fail to move file');
            } catch (e) {
    ...
    describe('#getFileHistory', () => {
        ...
        it('should not return history if file path without filename', async () => {
            try {
                await ctx.service.file.getFileHistory(repo.path);
                assert.fail('should fail to delete file');
            } catch (e) {
                assert(e.message === 'invalid file path');
            }
    ...
    describe('#getFileInfo', () => {
        ...
        it('should not return info if file path without filename', async () => {
            try {
                await ctx.service.file.getFileInfo(repo.path);
                assert.fail('should fail to delete file');
            } catch (e) {
                assert(e.message === 'invalid file path');
            }
    ...
    describe('#getFileRawData', () => {
        ...
        it('should not return raw data if file path without filename', async () => {
            try {
                await ctx.service.file.getFileRawData(repo.path);
                assert.fail('should fail to delete file');
            } catch (e) {
                assert(e.message === 'invalid file path');
            }
